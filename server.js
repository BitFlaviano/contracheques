...
async function processarPdfPorUsuario(req, res, bucket) {
    try {
        ...
        if (smtpConfigurado()) {
            try {
                console.log('SMTP configurado — verificando conexão...');
                ...
                console.log('SMTP OK — enviando email de contracheques...');
 
                // Baixar cada arquivo salvo e anexar no email
                ...
                await transporter.sendMail({
                    from: `"Portal Kidverte" <${process.env.SMTP_USER}>`,
...
app.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!smtpConfigurado()) {
                console.warn("SMTP não configurado — email de atestado NÃO enviado.");
