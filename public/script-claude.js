...
async function enviarDocumentos() {
    ...
    try {
        ...
        for (const file of arquivosSelecionados) {
            ...
            const res = await fetch('/upload-documento', {
                method: 'POST',
                headers: {
            ...
            try {
                ...
                throw new Error('Servidor retornou uma resposta inválida. Verifique se a rota de upload está ativa.');
            }
            ...
            documentos.push({
                tipo: data.bucket || 'documento',
                confianca: 1,
                nome: data.pasta || '-',
                periodo: `${data.paginas_processadas || 0} página(s), ${data.paginas_ignoradas || 0} ignorada(s)`,
                nomeArquivo: file.name,
                observacoes: data.status_email ? `Email: ${data.status_email}` : ''
            });
        }

        // Exibir resultados
        exibirResultados(documentos);
